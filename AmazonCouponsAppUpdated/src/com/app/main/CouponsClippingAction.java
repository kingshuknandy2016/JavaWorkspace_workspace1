...
public class CouponsClippingAction extends BaseExample {
 ...
 public static WebDriver DriverFinder(String st){
	 System.out.println("Inside Driver function");
	  WebDriver driver1=null;
	  if(st.equalsIgnoreCase("Firefox")){ 		
	...
	public  int Clipping(String st ){
		
		
		WebDriver driver=DriverFinder(st);
		...
		for ( i=1;i<=totalcpnhd;i++){
			...
			for (int j = 0; j <=cpnperpg; j++){				
				...
				for(WebElement cpntoclick:CpnListNtHidden){
					cpntoclick.click();
					count++;
                    test.log(LogStatus.INFO, "Total '"+count+"' coupon clicked(logStatus, details)");
